		...
		if(	EVPPImode == WITH_EVPPI){
			...
			for(int myParameter = 0; myParameter < NUMBER_OF_MODEL_PARAMETERS; myParameter++){
				for(unsigned int innerloop = 0; innerloop <  innerloopSize; innerloop++){
					...
					myCEModel.CalculateNMB_EVPPI(cRatio);
				}

				//Calculate Population EVPPI 
				double evppi = myCEModel.CalculatePopulationEVPPI(innerloopSize);
				cout << "EVPPI for the parameter " << ParameterNames[myParameter] << " = " << evppi << endl;
