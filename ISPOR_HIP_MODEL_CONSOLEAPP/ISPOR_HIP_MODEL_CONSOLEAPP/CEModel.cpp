...
CEModel::CEModel(bool Mode,
	...
	this->VOI_RESULTS_EVPPI = new double[this->NMB_SIZE1];
	memset(this->VOI_RESULTS_EVPPI,0,sizeof(double)*this->NMB_SIZE1);
}
...
double CEModel::CalculateNMB(long unsigned int myWTP){
	...
	memset(this->VoI_RESULTS,0,sizeof(double)*this->NMB_SIZE1);
	for(unsigned int runID=0; runID < this-> Number_of_Trials;runID++){
		memset(NMB_RESULTS[runID],0,sizeof(double)*NUMBER_OF_TREATMENT_ARMS);
	}
	double max =0;
...
void CEModel::CalculateNMB_EVPPI(long unsigned int myWTP){
	...
	for(unsigned int runID=0; runID < this->Number_of_Trials;runID++){
		...
		for(unsigned int treatment=0; treatment < NUMBER_OF_TREATMENT_ARMS;treatment++){
			NMB_RESULTS[runID][treatment] = this->myModelOutcome.Res[runID][treatment][Outcome_Result_QALY]*myWTP - this->myModelOutcome.Res[runID][treatment][Outcome_Result_COST];
			VOI_RESULTS_EVPPI[treatment] += NMB_RESULTS[runID][treatment];
			//find the maximum NMB per trial
			if(max<= NMB_RESULTS[runID][treatment]){max=NMB_RESULTS[runID][treatment];}
		}
		VOI_RESULTS_EVPPI[NUMBER_OF_TREATMENT_ARMS] += max;
	}
}

double CEModel::CalculatePopulationEVPPI(unsigned int innerloopSize){	
	//CALCULATE EVPPI
	double max =-1*DBL_MAX; //Max of the Mean NMBs
	for(unsigned int treatment=0; treatment < NUMBER_OF_TREATMENT_ARMS;treatment++){
		VOI_RESULTS_EVPPI[treatment] /= this->Number_of_Trials*innerloopSize;
		if(max<=VOI_RESULTS_EVPPI[treatment]){max = VOI_RESULTS_EVPPI[treatment];}
	}

	VOI_RESULTS_EVPPI[NUMBER_OF_TREATMENT_ARMS] /= this->Number_of_Trials*innerloopSize; //Mean of the Max NMB 
	double evppi = VOI_RESULTS_EVPPI[NUMBER_OF_TREATMENT_ARMS]-max;

	memset(this->VOI_RESULTS_EVPPI,0,sizeof(double)*this->NMB_SIZE1);
	return evppi*this->Cumulative_Effective_Population;
...
void CEModel::CleanUp(void){
	...
	//free  VoI_RESULTS_EVPPI array
	delete [] VOI_RESULTS_EVPPI;
